...
class LanguageModelGroupedQueryAttention(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if not self.sdpa:
            rank0_print("Warning: scaled dot product attention not available, using standard attention in LM.")

    def _build_document_additive_mask(
        self,
        ...
        *,
        ...
        # Document masking is only defined for full-sequence prefill.
        if document_ids is None or t_curr != t_kv:
            return None
        doc_ids = document_ids[:, :t_kv]
        if doc_ids.shape != (batch_size, t_kv):
            raise ValueError(
                f"document_ids shape must be {(batch_size, t_kv)}, got {tuple(doc_ids.shape)}."
            )

        doc_q = doc_ids[:, :t_curr]
        doc_k = doc_ids[:, :t_kv]
        key_valid = doc_k >= 0
        same_doc = doc_q.unsqueeze(-1) == doc_k.unsqueeze(1)
        query_valid = doc_q >= 0
        same_doc = torch.where(
            query_valid.unsqueeze(-1),
            same_doc & key_valid.unsqueeze(1),
            torch.ones_like(same_doc, dtype=torch.bool),
        )
        return (~same_doc).unsqueeze(1).to(dtype=dtype) * torch.finfo(dtype).min
    ...
    def forward_ori(
        ...
        explicit_causal = (T_curr == T_kv and T_curr > 1)
        ...
        document_attn_mask = self._build_document_additive_mask(
            document_ids,
            batch_size=B,
            t_curr=T_curr,
            t_kv=T_kv,
        ...
        if document_attn_mask is not None:
            additive_attn_mask = (
                document_attn_mask
                if additive_attn_mask is None
                else additive_attn_mask + document_attn_mask
            )
    ...
    def forward(self, x, cos, sin, attention_mask=None, block_kv_cache=None, document_ids: torch.Tensor | None = None):
        ...
        # block_kv_cache exists; in dual mode this cache is produced by LeftTower prefill.
        B, T_curr, C = x.size()
        T_cached = block_kv_cache["key"].size(2)
        ...
        # Dual prefill replacement:
        # - left cache already contains K/V for the full sequence
        # - right tower computes K/V from text decoder on the same full sequence
        # - we keep left K/V only at image-token positions, and right K/V elsewhere
        if needs_dual_prefill:
            if img_mask is None:
                raise ValueError("Dual prefill requires `img_mask` in block_kv_cache.")
            if T_cached != T_curr:
                raise ValueError(
                    f"Dual prefill expects cached length == current length, got cache={T_cached}, curr={T_curr}."
                )
            if img_mask.shape != (B, T_curr):
                raise ValueError(
                    f"Dual prefill `img_mask` shape must be {(B, T_curr)}, got {tuple(img_mask.shape)}."
                )

            mask_4d = img_mask.to(torch.bool).unsqueeze(1).unsqueeze(-1)  # [B,1,T,1]
            k_left = block_kv_cache["key"]
            v_left = block_kv_cache["value"]
            k = torch.where(mask_4d, k_left, k_rotated)
            v = torch.where(mask_4d, v_left, v_curr)
            block_kv_cache["key"] = k
            block_kv_cache["value"] = v
            block_kv_cache["needs_dual_prefill"] = False
        # Backward-compatible fallback for old prefix cache wiring.
        elif T_curr > T_cached:
            k_img = block_kv_cache["key"]
            v_img = block_kv_cache["value"]
            k = torch.cat([k_img, k_rotated[:, :, T_cached:, :]], dim=2)
            v = torch.cat([v_img, v_curr[:, :, T_cached:, :]], dim=2)
            block_kv_cache["key"] = k
            block_kv_cache["value"] = v
        else:
            # Concatenate with cached K, V
        ...
        # During prefill, apply causal masking over the full sequence.
        # During decode (T_curr=1), no additional causal mask is needed.
        is_prefill = explicit_causal
    ...
    def from_pretrained(cls, cfg):
                ...
                for hf_key, our_key in mapping.items():
                    ...
                    if hf_key in f.keys() and our_key in sd:
                        ...
                        if hf_key == 'model.embed_tokens.weight' and tensor.shape[0] != sd[our_key].shape[0]:
                            ...
                            rank0_print(f"Extending token embeddings from {tensor.shape} to {sd[our_key].shape}")
                            ...
                            rank0_print(f"Initialized {sd[our_key].shape[0] - tensor.shape[0]} new token embeddings")
                            sd['head.weight'].copy_(sd[our_key])  # Update the head weights as well
                        elif tensor.shape == sd[our_key].shape:
                            sd[our_key].copy_(tensor)
                        else:
                            rank0_print(f"Shape mismatch for {hf_key} -> {our_key}: {tensor.shape} vs {sd[our_key].shape}")
        ...
        for hf_key, our_key in mapping.items():
            if our_key not in loaded_keys:
                    if our_key in sd:
                        rank0_print(f"Warning: Key {our_key} not found in any safetensors file (HF key: {hf_key})")
                ...
                with safetensors.safe_open(filename=safetensors_file, framework="pt", device="cpu") as f:
                    if 'lm_head.weight' in f.keys():
                        ...
                        if lm_head.shape[0] != sd['head.weight'].shape[0]:
                            rank0_print(f"Extending LM head from {lm_head.shape} to {sd['head.weight'].shape}")
        ...
        rank0_print(f"Successfully loaded {cfg.lm_model_type} weights from safetensors. Model has {sum(p.numel() for p in model.parameters()):,} parameters.")
        return model
